...
library Diamond {
    ...
    bytes32 constant DIAMOND_INIT_SUCCESS_RETURN_VALUE = //TODO CHANGE THIS TO keccak256("diamond.saloon.init") - 1
    ...
    function _saveFacetIfNew(address _facet) private {
        ...
        if (selectorsLength == 0) {
            ds.facetToSelectors[_facet].facetPosition = uint16(
                ds.facets.length
            );
    ...
    function _addOneFunction(
        ...
        uint16 selectorPosition = uint16(
            ds.facetToSelectors[_facet].selectors.length
        );
    ...
    function _removeOneFunction(address _facet, bytes4 _selector) private {
        ...
        if (selectorPosition != lastSelectorPosition) {
            ...
            ds.selectorToFacet[lastSelector].selectorPosition = uint16(
                selectorPosition
            );
        }
    ...
    function _removeFacet(address _facet) private {
        ...
        if (facetPosition != lastFacetPosition) {
            ...
            ds.facetToSelectors[lastFacet].facetPosition = uint16(
                facetPosition
            );
        }
