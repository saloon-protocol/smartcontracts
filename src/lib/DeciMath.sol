...
contract DeciMath {
    ...
    uint256 constant TEN38 = 10**38;
    uint256 constant TEN30 = 10**30;
    uint256 constant TEN20 = 10**20;
    uint256 constant TEN19 = 10**19;
    uint256 constant TEN18 = 10**18;
    uint256 constant TEN17 = 10**17;
    uint256 constant TEN12 = 10**12;
    uint256 constant TEN11 = 10**11;
    uint256 constant TEN10 = 10**10;
    uint256 constant TEN9 = 10**9;
    uint256 constant TEN8 = 10**8;
    uint256 constant TEN7 = 10**7;

    // ln(2) - used in ln(x). 30 DP.
    uint256 constant LN2 = 693147180559945309417232121458;

    // 1 / ln(2) - used in exp(x). 30 DP.
    uint256 constant ONE_OVER_LN2 = 1442695040888963407359924681002;

    /***** LOOKUP TABLES *****/

    // Lookup table arrays (LUTs) for log_2(x)
    uint256[100] public table_log_2;
    uint256[100] public table2_log_2;

    // Lookup table for pow2(). Table contains 39 arrays, each array contains 10 uint slots.
    uint256[10][39] public table_pow2;
    ...
    // 18 Decimal places
    function decMul18(uint256 x, uint256 y)
        public
        pure
    ...
    function decDiv18(uint256 x, uint256 y)
        public
        pure
        returns (uint256 decQuotient)
    {
    ...
    // 30 Decimal places
    function decMul30(uint256 x, uint256 y)
        public
        pure
        returns (uint256 decProd)
    {
    ...
    // 38 Decimal places
    function decMul38(uint256 x, uint256 y)
        public
        pure
        returns (uint256 decProd)
    {
    ...
    function convert38To18DP(uint256 x) public pure returns (uint256 y) {
        uint256 digit = (x % TEN20) / TEN19; // grab 20th digit from-right
        return chopAndRound(x, digit, 20);
    }

    function convert38To30DP(uint256 x) public pure returns (uint256 y) {
        uint256 digit = (x % TEN8) / TEN7; // grab 8th digit from-right
        return chopAndRound(x, digit, 8);
    }

    function convert30To20DP(uint256 x) public pure returns (uint256 y) {
        uint256 digit = (x % TEN10) / TEN9; // grab 10th digit from-right
        return chopAndRound(x, digit, 10);
    }

    function convert30To18DP(uint256 x) public pure returns (uint256 y) {
        uint256 digit = (x % TEN12) / TEN11; // grab 12th digit from-right
    ...
    function chopAndRound(
        uint256 num,
        uint256 digit,
        uint256 positionOfChop
    ) public pure returns (uint256 chopped) {
        if (digit < 5) {
    ...
    function floor(uint256 x) public pure returns (uint256 num) {
        num = x - (x % TEN20);
        return num;
    }

    function countDigits(uint256 num) public view returns (uint256) {
        uint256 digits = 0;
    ...
    function powBySquare(uint256 x, uint256 n) public pure returns (uint256) {
        if (n == 0) return 1;
    ...
    function powBySquare38(uint256 base, uint256 n)
        public
        pure
        returns (uint256)
    {
  ...
  */
    function exp(uint256 x) public view returns (uint256 num) {
        uint256 intExponent; // 20 DP
        uint256 decExponent; // 20 DP
        uint256 coefficient; // 38 DP
    ...
    function log_2(uint256 x, uint256 accuracy)
        public
        view
        ...
        returns (uint256)
    {
        require(
            x >= TEN18 && x < 2 * TEN18,
            "input x must be within range [1,2["
        );
        uint256 prod = mul(x, TEN20); // make x 38 DP
        uint256 newProd = TEN38;
        uint256 output = 0;

        for (uint256 i = 1; i <= accuracy; i++) {
    ...
    function pow2(uint256 x) public view _onlyLUT3isSet returns (uint256) {
        require(
            x >= TEN20 && x < 2 * TEN20,
            "input x must be within range [1,2["
        );
        uint256 x_38dp = x * TEN18;
        uint256 prod = 2 * TEN38;
        uint256 fractPart = x_38dp % TEN38;
        uint256 digitsLength = countDigits(fractPart);

        // loop backwards through mantissa digits - multiply each by the Lookup-table value
        for (uint256 i = 0; i < digitsLength; i++) {
            uint256 digit = (fractPart % (10**(i + 1))) / (10**i); // grab the i'th digit from right
            ...
            uint256 term = table_pow2[37 - i][digit];
  ...
  */
    function ln(uint256 x, uint256 accuracy) public view returns (uint256) {
        require(x >= TEN18, "input must be >= 1");
        uint256 count = 0; // track
        uint256 TWO = mul(TEN18, 2);
    ...
    than simple successive division by 2. As max input of ln(x) is ~= 2^132, starting division at 2^30 yields sufficiently few operations for large x. */
        ...
        uint256 q = count * TEN30;
        uint256 output = decMul30(LN2, add(q, log_2(x, accuracy)));
  ...
  b^x = exp( x * (-ln(1/b)) ) = 1/exp(x * ln(1/b)).
   ...
   Thus, we avoid passing argument y < 1 to ln(y), and z < 0 to exp(z).   */
    function pow(uint256 base, uint256 x) public view returns (uint256 power) {
        if (base >= TEN18) {
            return exp(decMul18(x, ln(base, 70)));
        }

        if (base < TEN18) {
            uint256 exponent = decMul18(x, ln(decDiv18(TEN18, base), 70));
    ...
    function exp_taylor(uint256 x) public pure returns (uint256) {
        uint256 tolerance = 1;
        uint256 term = TEN18;
        uint256 sum = TEN18;
        uint256 i = 0;
